...
func NewTimeWheel(interval time.Duration, slotsNum int) *TimeWheel {
	...
	// windows 精度 不能小于 15ms， 所以windows下面最小间隔是 15ms这里必须单独分开！
	tickeTime := interval / time.Duration(slotsNum)
	if tickeTime < 1*time.Millisecond {
		nlog.Erro("windows 精度 不能小于 15ms， 所以windows下面最小间隔是 15ms")
		tickeTime = 1 * time.Millisecond
